...
for i in range (corridas):
	...
	txt = open(f"Rendimiento{i+1} scipy_false_longdouble.txt", "r")
...
plt.title("Rendimiento Matriz Invertida (scipy_false_longdouble)")
plt.ylabel("Tiempo transcurrido")
plt.grid(True)

for i in range (n):
	plt.loglog(Ns[i*n:(i+1)*n],dts_inv[i*n:(i+1)*n], linewidth = 1.0)
	plt.scatter(Ns[i*n:(i+1)*n],dts_inv[i*n:(i+1)*n])
